...
function mapBooking(b) {
  ...
  // Parse itemsJson back to array for frontend consumption
  if (mapped.itemsJson) {
    try { mapped.items = JSON.parse(mapped.itemsJson); } catch { mapped.items = []; }
  } else if (mapped.productId) {
    mapped.items = [{ productId: mapped.productId, productName: "Product", quantity: 1 }];
  } else {
    mapped.items = [];
  }
  return mapped;
...
exports.createBooking = async (req, res) => {
  try {
    ...
    // Use first productId for legacy compat, store all items as JSON
    const firstProductId = bookingItemsData[0]?.productId;
    ...
    const booking = await prisma.booking.create({
      data: {
        ...
        itemsJson: JSON.stringify(bookingItemsData),
        lat: location?.lat,
        lng: location?.lng,
      }
    });
...
exports.updateBookingStatus = async (req, res) => {
  try {
    ...
    // Parse items
    let items = [];
    try { items = JSON.parse(booking.itemsJson || "[]"); } catch {}
